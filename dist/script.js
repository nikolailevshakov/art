...
var accordion = function accordion(triggersSelector) {
  var btns = document.querySelectorAll(triggersSelector); // accortdion mostly with js
  ...
  }); // accordion with css selectors mostly    
...
var calc = function calc(size, material, options, promocode, result) {
  ...
  var calcFunc = function calcFunc() {
    // получаем значение value(строку) из html файла
    sum = Math.round(+sizeBlock.value * +materialBlock.value + +optionsBlock.value); // обязательный выбор определенных значений
...
var drop = function drop() {
  // события: drag, dragend, dragenter, dragexit, dragleave, dragover, dragstart, drop
...
var forms = function forms() {
  var form = document.querySelectorAll('form'),
      ...
      upload = document.querySelectorAll('[name=upload]'); // валидация, что вводишь
  ...
  upload.forEach(function (item) {
    item.addEventListener('input', function () {
      ...
      var dots;
      var arr = item.files[0].name.split('.'); // Делим название на массив из названия и расширения
  ...
  form.forEach(function (item) {
    ...
    item.addEventListener('submit', function (event) {
      ...
      event.preventDefault(); // создаем блок для отображения Message

      var statusMessage = document.createElement('div');
      statusMessage.classList.add('status'); // Засовываем статус сообщение в родитель инпута

      item.parentNode.appendChild(statusMessage); // скрываем форму со страницы, но не удаляем

      item.classList.add('animated', 'fadeOutUp'); // форма исчезает, чтобы страница не поплыла из-за наличия невидимого элемента

      setTimeout(function () {
        item.style.display = 'none';
      }, 400);
      var statusImg = document.createElement('img'); // добавляем путь до картинки-сообщения

      statusImg.setAttribute('src', message.spinner);
      statusImg.classList.add('animated', 'fadeInUp'); // помещаем картинку внутрь созданного дива

      statusMessage.appendChild(statusImg); // создаем див для тексти и помещаем внутрь того же дива под картинкой

      var textMessage = document.createElement('div');
      textMessage.textContent = message.loading;
      statusMessage.appendChild(textMessage); // Для получения всей инфу из инпутов в спец структуру FormData

      var formData = new FormData(item); //чтобы сгенерировать динамический путь, куда все отправлять в зависимости от родителя формы

      var api; // ищет выше по иерирархии, выдаст тру если найдет и фолс если нет
      // условие или для понимания куда отправлять картинку, куда текст

      item.closest('.popup-design') || item.classList.contains('calc_form') ? api = path.designer : api = path.question;
      console.log(api); // вернется промис с сервера
...
var mask = function mask(selector) {
  ...
  var setCursorPosition = function setCursorPosition(pos, elem) {
    ...
    if (elem.setSelectionRange) {
      elem.setSelectionRange(pos, pos); // Ручно полифил для ie
    } else if (elem.createTextRange) {
      var range = elem.createTextRange(); // граничные точки

      range.collapse(true); // конец и начало диапазона
  ...
  function createMask(event) {
    ...
    def = matrix.replace(/\D/g, ''),
        // Динамичное
    val = this.value.replace(/\D/g, ''); // фильтр на удаление символов
  ...
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    ...
    trigger.forEach(function (item) {
      item.addEventListener('click', function (e) {
        ...
        modal.style.display = 'block'; // при скролле страницы, скролится будет только модальное окно, не вся страница
    ...
    modal.addEventListener('click', function (e) {
      ...
      if (e.target === modal) {
        modal.style.display = 'none';
        document.body.style.overflow = 'initial';
        document.body.style.marginRight = "0px"; // используем классы bootstrap для того же 
...
// при наведении мыши, меняется изображение
...
var scrolling = function scrolling(upSelector) {
  var upElem = document.querySelector(upSelector); // появление и исчезание элемента при скроллинге
...
// при нажатии кнопки появляются еще блоки и исчезает кнопка


var showMoreStyles = function showMoreStyles(trigger, wrapper) {
  var btn = document.querySelector(trigger); // решаем с помощью стилей 
  // cards.forEach(card => {
  ...
  // });
  // решаем с бэкенд частью (из базы данных)

  btn.addEventListener('click', function () {
    //url где брать данные
...
var slider = function slider(slides, dir, prev, next) {
  ...
  showSlides(slideIndex); // перелистывание слайдов
  ...
  items[0].parentNode.addEventListener('mousleave', function () {
    activateAnimation();
  });
  activateAnimation();
};
